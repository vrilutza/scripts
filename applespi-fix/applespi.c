			    ...
			    struct touchpad_protocol *t)
{
	...
	int i, j, n;
	int finger_idx[MAX_FINGERS];
	int x, y;
	long dx, dy;
	/* Previous-frame positions — persists across calls for velocity check */
	static struct input_mt_pos prev_pos[MAX_FINGERS];
	static int prev_n;
	...
	for (i = 0; i < t->number_of_fingers; i++) {
		...
		x = le16_to_int(f->abs_x);
		y = tp_info->y_min + tp_info->y_max - le16_to_int(f->abs_y);

		/* Drop coordinates outside the physical touchpad area. */
		if (x < tp_info->x_min || x > tp_info->x_max ||
		    y < tp_info->y_min || y > tp_info->y_max)
			continue;

		/*
		 * Velocity filter: if we have previous-frame positions and
		...
		if (prev_n > 0 && prev_n == t->number_of_fingers) {
			...
			for (j = 0; j < prev_n; j++) {
				...
				dy = y - prev_pos[j].y;
				if (dx * dx + dy * dy <=
				    (long)APPLESPI_MAX_DELTA * APPLESPI_MAX_DELTA) {
					glitch = false;
					break;
				}
			}
	...
	/* Save positions for next-frame velocity check */
	prev_n = n;
	for (i = 0; i < n; i++)
		prev_pos[i] = applespi->pos[i];

	input_mt_assign_slots(input, applespi->slots, applespi->pos, n, 0);

	for (i = 0; i < n; i++)
		report_finger_data(input, applespi->slots[i],
				   &applespi->pos[i], &t->fingers[finger_idx[i]]);
