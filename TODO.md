...
| **Stare de bază** | Hardware-ul e funcțional. Margini: 2 probleme cronice (BT, WiFi), 1 **nediagnosticată** (opriri spontane), 1 la upstream (cameră), 1 fizică (termic). La upstream, verificat prin API pe 24 sep: **treisprezece MR-uri PipeWire acceptate** (ultimul !3013, integrat pe 22 sep; trei dintre ele și în ramura `1.6`), **cinci PR-uri de driver integrate**, un MR (!2951) plus issue-ul #5481, **douăsprezece PR-uri de driver** și **două la `facetimehd-firmware`** încă deschise. 2 laptopuri de test. Tabloul complet: [secțiunea 0.1](#01-rapoarte-trimise-upstream--tablou). |
...
| 3 | Cameră — partajare de buffere fără `SPA_META_Busy` (aplicațiile îngheață) | 🔵 upstream | **treisprezece patch-uri PipeWire + cinci de driver acceptate în master** (trei dintre cele PipeWire și în ramura `1.6`); un PipeWire (!2951), issue-ul #5481, douăsprezece PR-uri de driver și două la `facetimehd-firmware` încă deschise *(verificat prin API, 24 sep)* | [3](#3--camera-facetime-hd--partajare-de-buffere-nesigură) |
...
| **8** | **Opriri spontane, fără urmă în jurnal** | 🟠 **recidivă ×2 pe 24 sep; pistă hardware** | **prima moarte cu punct fixat**, prin netconsole: la `Virtual VTT enabled`, în bringup-ul DDR40, încărcând modulul **stock**. 15 încărcări în ziua aia, una fatală. A doua moarte, în altă etapă (pornirea firmware ISP), **cu #340 încărcat** — deci patch-ul nu le închide. **`pstore` e gol la fiecare moarte ⇒ niciun panic**, zero MCE, zero termic: mașina nu se prăbușește, se oprește. Trei din cele cinci morți vechi au fost **noaptea, cu mașina inactivă**, deci camera nu e explicația generală. **Pistă nouă: bateria — 1446 de cicluri, 62 % din capacitatea de proiect.** Dacă ține, nu există patch, e înlocuire de piesă. **25 sep, prima zi de logger:** 115 eșantioane cu `ac=0` în 3 episoade (kernelul nu le loghează deloc), iar rezistența internă a bateriei iese **≈320 mΩ** din 129 de puncte de descărcare, față de 100–200 tipic. La un vârf de 5 A căderea ar duce tensiunea la ~10,7 V, lângă pragul de tăiere al unui 3S. **25 sep 16:17 — oprirea a fost prinsă cu logger-ul activ, și ipoteza bateriei NU se confirmă:** ultimele 14 eșantioane arată `ac=1`, baterie plină, **12,397 V stabil la 2 mV, curent 0**. Nicio cădere, niciun episod pe baterie. Punctul de moarte: `Full memory verification succeeded`, în timpul unei încărcări de modul. **Toate trei morțile cu date bune sunt în bringup-ul ISP**, `pstore` gol la toate. Rezervă: la 1 s nu se vede un tranzitoriu de milisecunde. JURNAL §46, §55, §58 | [8](#8--opriri-spontane--cauză-nedeterminată) |
...
| [facetimehd #350](https://github.com/patjak/facetimehd/pull/350) | driverul tăcea complet când lipsea fișierul de calibrare: camera merge, culorile sunt greșite, utilizatorul nu află nici că lipsește ceva, nici cum se cheamă | 🔵 **deschis 24 sep**, `MERGEABLE`, un commit `b316e7d`, +8/−2. Cele două căi care renunță devin `dev_info`, iar succesul numește fișierul **după** ce ISP-ul l-a acceptat. Probat pe hardware în trei build-uri separate; `checkpatch --strict` curat |
| [facetimehd #351](https://github.com/patjak/facetimehd/pull/351) | alegerea set file-ului pentru Air compară `DMI_BOARD_NAME`, care pe firmware Apple ține identificatorul de placă — deci nu se potrivește niciodată | 🔵 **deschis 24 sep**, `MERGEABLE`, un commit `4506ac6`, +5/−5. **Calea corectată e NETESTATĂ** (n-avem Air), scris explicit în descriere. Raportorul din #349 are MacBookAir6,2 și a fost rugat să testeze |
| [facetimehd-firmware #14](https://github.com/patjak/facetimehd-firmware/pull/14) | uneltea extrăgea set file-urile doar din `AppleCameraAssistant`; driverul Boot Camp 041-89042 le are pe toate unsprezece | 🔵 **deschis 24 sep**, `MERGEABLE`, două commit-uri `7edfb6d`+`530858f`. Cele 11 sha256 din a doua sursă ies identice cu tabelul lor, deci sursele se validează reciproc. Al doilea commit șterge fișierele pe jumătate extrase la un hash greșit |
| [facetimehd-firmware #15](https://github.com/patjak/facetimehd-firmware/pull/15) | mesajul de nepotrivire tipărea `${firm_hash}` în loc de `fw_hash`, deci **fără hash**; plus un „then" rămas ca argument | 🔵 **deschis 24 sep**, `MERGEABLE`, două commit-uri `026ac9b`+`1814328`. Ieșirile comparate octet cu octet cu versiunea dinainte. Se ciocnește cu #14 în două locuri mici |
...
| [facetimehd #340](https://github.com/patjak/facetimehd/pull/340) | bucla de așteptare a PLL-ului S2 și propagarea erorii de inițializare | 🟠 **`patjak` a răspuns pe 24 sep, 08:34** — primul semn de la el din 4 sep. Cere o verificare pe hardware: *„Can you verify that S2_PLL_CMU_STATUS_LOCKED bit goes from 0 to 1 and not 1 to 0 on real hardware?"* Numele registrului sunt presupunerile lui inițiale. **Întrebarea lovește exact primul commit** (`1e99880`), cel care inversează condiția. **MĂSURAT în aceeași zi, 16:20–16:40: bitul face 0 → 1 la 21–32 µs după resetul PLL, de 5 ori din 5, niciodată invers.** A/B alternat, 10 încărcări: master 4/5 „Failed to lock" (bitul era deja pus, bucla lui așteaptă să cadă), #340 **5/5 „locked after 10 us"**. Camera pornește în ambele cazuri. Control: alte două registre citite înainte de reset dau valori distincte și plauzibile, deci `0xc902c902` nu e gunoi de magistrală. **Bancul auditat și refăcut** (rezoluția era 25 µs, nu 10; `t0` era luat după un printk): v2 măsoară din chiar scrierea care pornește PLL-ul, cu 1 µs pe citire. **Runda 2, 15 încărcări: bit 14 urcă la 6–7 µs, bit 15 urcă 0 → 1 la 16–18 µs, zero tranziții inverse.** A/B repetat, total 13 perechi: master 11 eșecuri, #340 13 din 13; camera a pornit în toate 26. ✅ **RĂSPUNS TRIMIS** în firul lui de review, `pull/340#discussion_r4094456463`. Dovezi: `rezultate/pll-polaritate-24sep/`, JURNAL §41–43 |
| [facetimehd](https://github.com/patjak/facetimehd/pulls) — **deschise** | #331 (enumerare reală), #334 (AE în 200 ms), #338 (decupare centrată), #340 (bucla PLL), #342 (`CREATE_BUFS`), #343 (unitățile ratei + implicitul și `G_PARM`), #344 (AE coboară rata), #345 (plafonul de tampoane), #347 (`VIDIOC_G_SELECTION`) | 🔵 **nouă deschise** (plus #348, #350 și #351 — rânduri separate mai sus, deci **douăsprezece** în total), **testate fiecare separat pe hardware pe 19 sep**: toate conform descrierilor, față de master; toate **revizuite și republicate pe 17 sep** (14 commit-uri în 9 teme, `mergeable`/`clean`); pe seria completă `v4l2-compliance` **57/57** fără avertismente (16 sep). **Cinci cu corecții noi de cod:** #340 (eroarea DDR propagată până la inițializarea principală), #342 (limitele de locuri și de memorie la `CREATE_BUFS`), #343 (fără pierdere de precizie prin ms, conversii protejate, schimbarea refuzată în timpul capturii), #344 (starea se schimbă doar după ce firmware-ul acceptă comanda; ramura conține întregul #343), #345 (indexul de debug reparat înainte de plafonul 8). **Patru cu cod neschimbat**, doar explicații și istoric mai clare: #347, #331, #334, #338. Niciun răspuns de la `patjak` până pe 18 sep (ultima lui activitate: 4 sep). Vezi [3.3p](#33p--1618-septembrie--seria-revizuită-și-republicată-bancurile-auditate-ciornele-pipewire-ținute-local) |
...
### 3.3e 🔵 24 august — `wtaymans` a integrat trei MR-uri; patru comituri de-ale noastre în `pipewire/master`

Prima mișcare de la un **Owner** pe MR-urile astea. Nu aprobare, nu comentariu — merge direct.

| Ora (UTC) | MR | Ce s-a întâmplat | Comit în master |
|---|---|---|---|
| 12:58 | **!2965** | integrat, **fast-forward** — sha neschimbat | `c4309f0eb` *clamp a fraction that does not fit a GstFraction* + `acea30afa` *keep the value a source prefers when converting a choice to caps* |
| 13:02 | **!2964** | integrat, **fast-forward** — sha neschimbat | `cefb4e926` *report the step of a stepwise frame size, not the maximum* |
| 13:04 | **!2954** | integrat, **rebazat** `f5589a4e` → `6734d69c` | `6734d69c8` *do not expose compound controls as props* |

**Rebazarea lui !2954 n-a schimbat nimic în conținut** — verificat, nu presupus: `md5` peste diff-ul
întors de API pentru ambele sha-uri = `59c702f5f5b1b41576ab53cc63125fed`, identic; `md5` peste mesaj
= `8c36efd03b209c399ee06915bb16ace7`, identic; singura diferență e părintele (`f03a55d7f` →
`cefb4e926`). Autorul rămâne **Viorel Cernate** pe toate patru comiturile. Zero comituri de merge —
proiectul merge liniar.

**Un lucru care contrazice ipoteza de ieri:** !2965 **nu** fusese aprobat de `pobrn` (el aprobase doar
!2954 și !2964) și totuși a intrat primul. Deci aprobarea lui e semnal de recenzie, nu poartă de
merge — exact cum scria și `approvals_required: None`. Nu trebuie citită ca o condiție.

**Ce a rămas deschis: !2935, !2950, !2951, !2963.** Toate patru arată acum
`detailed_merge_status: need_rebase` — nu e conflict, e doar că master-ul a avansat sub ele
(`f03a55d7` → `69187d4cd`) și proiectul cere istorie liniară. Testat local, nu dedus: fiecare dintre
cele patru se aplică **curat** prin `cherry-pick` pe `69187d4cd`.

**!2963 nu a devenit redundant prin merge-ul lui !2954**, deși amândouă sunt în `v4l2-utils.c`:
!2954 atinge `spa_v4l2_enum_controls()`, !2963 atinge `spa_v4l2_update_controls()` — funcții
diferite. Aplicat pe master-ul de azi: 10+/10−, iar `res` rămâne folosit de `spa_v4l2_open()`, deci
nici avertisment de variabilă nefolosită.

**Restul: zero.** 👍-urile lui `rmader` tot cele trei din 22 aug, niciunul nou. Pe !2950 cele două
fire ale lui `pobrn` sunt **tot nerezolvate** și n-a răspuns la nota editată pe 23 aug. `#5431` fără
notă nouă din 18 aug, `#5363` din 15 aug. Zero reviewer sau assignee pe vreunul din cele patru
rămase.

**facetimehd: nicio mișcare pe ale noastre, dar doi vecini noi în depozit.**

| Loc | Stare la 24 aug |
|---|---|
| `#328`…`#334` | toate `OPEN`, `mergeable: clean`, zero review-uri, zero comentarii inline, zero comentarii noi pe fir |
| master | `364b1c6`, **30 iunie**, neschimbat |
| `patjak` | ultimul comentariu **17 aug**, neatins |
| serie `linux-media` | RESEND-ul din 20 aug, toate cele 5 patch-uri `state: new`, **zero comentarii** (verificat pe `/api/patches/<id>/comments/`, nu doar starea) |

Cei doi vecini ating amândoi `fthd_v4l2.c`, adică fișierul din 5 din cele 7 PR-uri ale noastre:

* **#335** (`vicamo`, 20 aug) — pune `#if LINUX_VERSION_CODE < KERNEL_VERSION(4,2,0)` în jurul
  completării descrierii de format, fiindcă `strncpy` dispare din kernel în 7.2. Atinge doar
  `fthd_v4l2_ioctl_enum_fmt_vid_cap()`, funcție pe care **niciunul** din PR-urile noastre n-o atinge.
* **#337** (`3within1`, 24 aug 13:18) — refuză suspend-ul cât camera e deschisă: `int users` devine
  `atomic_t`, `open`/`release` proprii, `kfree(dev_priv)` și `pci_set_drvdata(NULL)` la teardown,
  eroarea de probe propagată din resume. Niciunul din cele șapte diff-uri ale noastre nu pomenește
  `users` (`grep -c` = 0 pe toate șapte). În practică nu ne schimbă nimic oricum — suspend-ul e
  won't-fix permanent pe mașina asta.

Testate toate **14 perechi** local (`merge` pe worktree, nu judecată din ochi): **zero conflicte**.

**Verificarea proprie, iar: cele 11 comituri ale noastre, live vs local.** `patch-id` per comit,
nu pe diff-ul cumulat — **11 din 11 identice**. Prima comparație pe care am făcut-o a raportat patru
PR-uri „diferite"; era artefactul meu, comparam un `patch-id` cumulat cu câte unul per fișier de
patch. Diferență reală a existat totuși, în altă parte: exportul local `patches/pr-live/` pentru
**#329** și **#334** era cel **dinaintea** re-măsurării — purta tabelele vechi (`90.77` la #329,
...
Probat pe hardware în trei variante separate: `no set file for sensor 0005 0248` (master curat,
...
Matricea de probe a uneltei rulată integral din nou: 5/5.
...
**19 sep — o zi lungă, totul măsurat înainte de trimitere** (jurnal: `pipewire-5363/JURNAL.md` §19–23):
- **Garda din `mmap_read()`**, separată de `filter.h`, a devenit **!3013**. Înainte a fost testată local pe ambele
  mașini (suita, ASan/UBSan, patru treceri, gdb, reproducere verbatim) și pe 1.6.9, unde e nevoie și de !2934.
  După primul review, comentariul din cod și mesajul au fost reformulate: e un ocol pentru un driver neconform.
- **!2951:** A/B-ul în Snapshot e făcut, apoi rebase și comentariu (vezi tabloul).
- **Testul de calibrare 1571 e făcut:** cu `1571_01XX.dat`, tenta pe foaia albă e cu 46% mai aproape de neutru,
  iar dungile scad cu 10%. Patch-ul facetimehd pentru senzorul `0x248` și nota de wiki (extragerea din Boot Camp,
