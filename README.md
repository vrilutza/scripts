...
- Firmware auto power-on off (optimize step 17: binary `AutoBoot=%00` — the switch non-T2 Intel
  Macs honor — plus ASCII `auto-boot=false`) — since the machine never sleeps, the way to carry it
  around is a plain shutdown; with the factory default it would then power itself back on at
  lid-open or power-connect (even inside a bag). Now it stays off until the power button.
